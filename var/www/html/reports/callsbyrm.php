...
$sttime = str_replace("T", " ", $var1);
$etime = str_replace("T", " ", $var2);
$sql = "select * from reports where starttime between '$sttime' and '$etime'$resi;";
echo "<table style='width:65%' align='center'";
echo "<h1><tr class='resident'><td>"."Start Time"."</td><td>"."End Time"."</td><td>" ."Time Active" ."</td><td>". "Call Type"."</td><td>". "Room" . "</td><td>". "Bed"."</td></tr></h1>";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
         ...
         $hours = intval($total_minutes/60);
         $timeFormat = sprintf('%02d:%02d:%02d', $hours, $minutes, $seconds);
         echo "<h1><tr class='resident'><td>".$row[starttime]."</td><td>".$row[endtime]."</td><td>".$timeFormat."</td><td>" .$row[type]."</td><td>" .$row[room]. "</td><td>" .$row[bed]. "</td></tr></h1>";
    }
} else {
     echo "<table style='width:100%'>";
        echo "<h1><tr><td><span class='label label-success label-large'>"."No Entries"."</span></td></tr></h1>";
